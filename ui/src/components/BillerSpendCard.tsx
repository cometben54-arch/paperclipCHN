    ...
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> 输入
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> 输出
              {" · "}
              {row.providerCount} 个供应商
              {" · "}
              {row.modelCount} 个模型
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        ...
        <div className="text-xs text-muted-foreground">
          {row.apiRunCount > 0 ? `${row.apiRunCount} 次计量运行` : "0 次计量运行"}
          {" · "}
